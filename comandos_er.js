...
// lógica para baixar o excel, selecionado as tags para copiar
    ...
    const label_1 = document.getElementById('texto_er01'); // Vincula à tag <label id="texto_er01">
    // Seleciona o elemento <input> pelo ID
    const input_1 = document.getElementById('anomalias_er01'); // Vincula à tag <input id="anomalias_er01">
    // Seleciona o elemento <input> pelo ID
    //const nqm01 = document.getElementById('nqm_q1_er01'); // Vincula à tag <input id="anomalias_er01">
    // Seleciona o elemento <input> pelo ID
    const nqm_q3 = document.getElementById('nqm_q3_er01'); // Vincula à tag <input id="anomalias_er01">
    // Seleciona o elemento <input> pelo ID
    const desvios_in = document.getElementById('desvios_internos_er01'); // Vincula à tag <input id="anomalias_er01">
    // Seleciona o elemento <input> pelo ID
    const desvios_for = document.getElementById('desvios_fornecedor_er01'); // Vincula à tag <input id="anomalias_er01">
    // Seleciona o elemento <input> pelo ID
    const ordens_produzidas = document.getElementById('ordens_produzidas_er01'); // Vincula à tag <input id="anomalias_er01">
...
//***********************************************************************************************************
// Somando 
//var resultado_somados_preenchidos = conteudoInput_1 + conteudo_nqm01 + conteudo_nqm_q3 + conteudo_desvios_in + conteudo_desvios_for + conteudo_ordens_produzidas
// ------------------------
...
//************************************************************************************************************
    ...
    const worksheet = XLSX.utils.aoa_to_sheet([
        [titulo_semana,tituloLabel, tituloInput,//titulo_nqm01,
            titulo_nqm_q3,titulo_desvios_in,titulo_desvios_for,
            titulo_ordens_produzidas,titulo_ppm,titulo_in_qualidade
        ], // Adiciona os títulos personalizados
    
    
        [conteudo_nsemana,conteudoLabel_1, conteudoInput_1,//conteudo_nqm01,
            conteudo_nqm_q3,conteudo_desvios_in,
            conteudo_desvios_for, conteudo_ordens_produzidas,resultado_ppm, resultado_final
        ] // Adiciona os conteúdos lado a lado
    ]); // Cria uma nova planilha com os conteúdos
    ...
    function s2ab(s) {
        const buf = new ArrayBuffer(s.length); // Cria um buffer de array
        const view = new Uint8Array(buf); // Cria uma visão de array de 8 bits
        for (let i = 0; i < s.length; i++) {
            view[i] = s.charCodeAt(i) & 0xFF; // Converte cada caractere para 8 bits
        }
    ...
    elementoDownload.download = 'Relatório não conformidades ER.xlsx'; // Define o nome do arquivo para download
...
var subject = "Resumo Semanal - Qualidade ER"; // Define o assunto do email
